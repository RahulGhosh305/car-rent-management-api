...
const getCars =  catchAsync( async(req, res) => {
    const cars = await CarRentModel.find({status:{ $ne : CarRentStatus.deleted}}, {name : true, body: true, color: true, seat: true})
    return apiResponse(res, httpStatus.OK, {data: cars})
})

const getCar = catchAsync( async(req, res) => {
    const car = await CarRentModel.findOne({_id: req.params._id}, {name : true, body: true, color: true, seat: true})
    return apiResponse(res, httpStatus.OK, {data : car})
})

const addCar = catchAsync( async(req, res) => {
    const {name, body, color, seat} = req.body
    const newAddCar = new CarRentModel({name, body, color, seat})
...
const updateCar = catchAsync( async(req, res) => {
    const {name, body, color, seat} = req.body
    const modify = await CarRentModel.updateOne({_id: req.params._id}, {name , body, color, seat})
